...
public class ProcessOrder extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session.getAttribute("name") != null) {
			...
			FOODY_USERS user = null;
			try {
				// Fetching CID for the given User Mail ID
				user = UsersDAO.getInstance().getUserFromEmail(identifierMail);
			} catch (Exception e) {
				log.error(e);
			}
			...
			if (user.getAddress() != null) {
				...
				try {
					...
					for (CartItem temp : data) {
						...
						FOODY_ORDER_DETAILS details = new FOODY_ORDER_DETAILS(newOrder, temp.getItemName(),
								temp.getItemCount(), Float.parseFloat(price[price.length - 1]));
						orderDetailsInstance.insertDetailsForOrderID(details);
					}

					pw.println(
							"<html><head></head><body onload='clearLocal()'><script type='text/javascript'>function clearLocal(){ window.alert(\" Your Food order has been placed Successfully and your Order ID is "
									+ Order_ID
									+ " \"); localStorage.clear(); window.location =\"./index\";}</script></body></html>");
				...
				log.warn("User Address Can't be Found --> " + user.toString() + "\nRedirecting to MyProfile");
				pw.println(
						"<html><head></head><body onload='add()'><script type='text/javascript'>function add(){ window.alert(\" Unable To Fetch ADDRESS For User. Please Try Updating it first \"); window.location =\"./myProfile\";}</script></body></html>");
			}
		} else {
	 ...
	 * 
	 * @param input = Input String [{A,B},{X,Y}]
